...
const initializeAdmin = async () => {
  try {
    ...
    const existingAdmin = users.find(u => u.username === 'Dababyq');
    
    if (!existingAdmin) {
      const adminUser = {
        id: uuidv4(),
        username: 'Dababyq',
        password: await hashPassword('Dababy19'),
        is_admin: true,
...
// Cache for admin initialization
...
exports.handler = async (event, context) => {
  ...
  // Pre-connect to database for faster response
  if (!dbConnection) {
    dbConnection = await storage.connectDB();
  }
  ...
  try {
    ...
    switch (action) {
      ...
      case 'register':
        ...
        break;
      case 'validateKey':
        result = await handleValidateKey(payload);
        break;
...
async function handleValidateKey(payload) {
  const { key, hwid } = payload;
  ...
  try {
    ...
    const foundKey = keys.find(k => k.key_string === key);
    ...
    if (!foundKey.is_active) {
      return { ok: false, error: 'Key is inactive' };
    }
    ...
    // Check HWID ban
    if (foundKey.hwid_banned) {
      return { ok: false, error: 'Key is banned' };
    }
    
    // HWID binding logic
    if (foundKey.hwid) {
      if (foundKey.hwid !== hwid) {
        return { ok: false, error: 'Key is bound to different hardware' };
      }
    } else {
      // First time using this key - bind to current HWID
      foundKey.hwid = hwid;
    ...
    return {
      ...
      data: {
        key: foundKey,
        is_active: foundKey.is_active,
        hwid_banned: foundKey.hwid_banned || false,
